import { a as u, d as h, e as v, j as f } from "./chunk-ZXYWZMUN.mjs";
import { ee as C, wd as g } from "./chunk-3J64XJNF.mjs";
import { b as a } from "./chunk-4JY5UMT2.mjs";
import { h as T } from "./chunk-KPMZENE5.mjs";
var d = T("remote:verify");
function S(s, e, t, o) {
  let r = s.getPartialDocumentForPageIds(t),
    i = [],
    l = d.isLoggingTraceMessages() ? [] : void 0,
    c = g(r, null, { extraChecksAndFixes: !0, errors: i, warnings: l });
  (f("parsingRootNode"), a(c, () => `error loading root node: ${r.id} ${JSON.stringify(i)}`));
  for (let p of c.children ?? []) p.children && p.children.length > 0 && h(p, i);
  (i.length > 0 &&
    d.warn(
...
`)
  ...
  let n = C.createByAdoptingRoot(c, o);
  return (
    n.loadReplicasAndCodeComponents(n.root),
    f("parsingReplicasExpansion"),
    n.hasUncommittedChanges() && (n = n.commit(e)),
    n
  );
}
function I(s, e) {
  ...
  if (!t) return !0;
  let o = [],
    r = g(t, s.root.id, { extraChecksAndFixes: !0, errors: o });
  return (
    a(r, () => `error loading page node: ${t.id} ${JSON.stringify(o)}`),
    h(r, o),
    o.length > 0 &&
      d.warn(
        "warnings loading server tree: " +
...
function P(s, e, t) {
  return (
    ...
    (s.editClosed = !1),
    (s.isViewOnly = !1),
    s.root.children.forEach((o, r) => {
      let i = t.get(o.id);
      i && (s.remove(i.id), s.insertNode(i, s.root.id, r));
    }),
    (s.inEditor = !1),
...
var m = class s {
  ...
  A = class {
    ...
    buildCompleteTree() {
      (a(this.pagesToAssemble.length === 0, "must be done with assembleOnePage"),
        this.setupAssemblerTreeStateIfNeeded(),
...
export { S as a, I as b, P as c, m as d, A as e };
//# sourceMappingURL=https://app.framerstatic.com/chunk-72GLORFR.mjs.map
